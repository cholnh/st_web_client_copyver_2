...
import 'package:stwebclientcopyver2/domains/user/enum/auth_code_state.dart';
...
class SignUpModel with ChangeNotifier {
  ...
  Future<User> postUser() async {
    try {
      ...
      int dIdx = prefs.getInt(s.idxDeliverySite);
      ...
      return await _signRepository.postUser(
        user: User(
          ...
          birth: birth,
          sex: sex,
          deliveryDetailSite: DeliveryDetailSite(idx: idxDeliveryDetailSite, idxDeliverySite: dIdx),
          password: password
      ));
    } catch(error) {
      return null;
